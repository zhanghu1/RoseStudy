...
import com.cantellow.lib.dao.UserDAO;
...
public class UserController {
    ...
    @Post("add")
    public String add(final Invocation inv, @Param("password2") final String password2,
            final User user) {
        ...
        final User userFromDB = this.userDAO.getByLoginName(user.getLoginName());
        if (userFromDB != null) {
    ...
    @Post("{id}/info/update")
    public String updateUser(final Invocation inv, @Param("id") final String id, final User user) {
        ...
        return "r:/lib/user/" + id;
    }

    @Post("{id}/password/update")
    public String updatePassword(final Invocation inv, @Param("id") final String id,
            @Param("old_password") final String oldPassword,
            @Param("password") final String password, @Param("password2") final String password2) {
        final User userFromDB = this.userDAO.get(Long.parseLong(id));
        inv.addModel("user", userFromDB);
        // 服务器端验证字符的合法性
